...
create view  imsdw_testing.qc_metadata_dx as
select * from d_metadata
where lower(level2_event_name) ~ 'diagnosis|problem'
order by level1_context_name, level2_event_name, level4_field_name
;
select * from d_metadata
where META_DATA_KEY in (3490, 5719)
;

create or replace view  imsdw_testing.qc_metadata_rx as
select distinct m.* 
from d_metadata m
join fact using (meta_data_key)
join b_material using (material_group_key)
join d_material using (material_key)
 where material_type='Drug'
 and  m.level1_context_name = 'EPIC'
 and   m.level2_event_name in ('Prescription','Medication Administration', 'Medication Order', 'Medication Reported')
order by level1_context_name, level2_event_name, level3_action_name, level4_field_name
 ;
...
with tmp as (
    ...
    where context_name like 'ICD%'
        ...
        and person_key>3 and encounter_key>3 and data_state_key=1
        --and context_diagnosis_code similar to '(C91.1|204.1)%'
)
/*
select icd
