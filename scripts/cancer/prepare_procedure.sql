...
--explore
-- source_procedure
create table ct.source_procedure as
select context_name, context_procedure_code, procedure_description
, count(*) records, count(distinct medical_record_number) patients
from dev_patient_info_pan.procedures
group by context_name, context_procedure_code, procedure_description
;
create view ct.v_source_procedure as
select * from ct.source_procedure
-- where procedure_description ilike '%transplan%'
order by procedure_description, context_name, context_procedure_code
;


select context_procedure_code, procedure_description
, count(distinct medical_record_number) patients
from dev_patient_info_mm.procedures
where context_procedure_code ilike '382%'
group by context_procedure_code, procedure_description
order by context_procedure_code, procedure_description
;
...
from _person dp
join ${dmsdw}.fact f using (person_key)
join ${imsdw}.d_calendar using (calendar_key)
join ${dmsdw}.d_metadata m using (meta_data_key)
