...
Requires:
   cohort, cplus_from_aplus
Results:
    latest_ecog, latest_karnofsky
 */
drop table if exists latest_ecog cascade;
...
drop table if exists latest_karnofsky cascade;
...
where row_number=1
;
create view qc_ecog as
select ecog_ps, count(distinct person_id)
from latest_ecog
group by ecog_ps
order by ecog_ps
;
create view qc_karnofsky as
