...
drop table if exists stage_plus;
...
join dev_patient_clinical_lca using (person_id)
...
select person_id
, nvl(stage_extrated, stage_imputed) as stage
, regexp_substr(stage, '^[0IV]+') stage_base
from stage_plus
;

select ct.assert (bool_and(stage='0' or stage like 'I%'), 'stage startswith 0 or I')
from select distinct stage from stage
;
...
select a.count, e.count, assert(a.count>e.count, 'imputing rescues some stage')
;
